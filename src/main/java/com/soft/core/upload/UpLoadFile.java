...
public abstract class UpLoadFile{
     ...
     * @throws Exception 
     */
    protected MesObject upload() throws Exception {
        InputStream inFile;
    ...
    public  void initUploadBean(String propertyFileName) throws Exception{
    	...
    	if((!SystemContext.getRealPath().equals(savePath))&&(uploadBean.getUploadDir()!=null)){
        	savePath = uploadBean.getUploadDir()+File.separator;
        	// 如果设置磁盘路径 
        ...
        if(uploadBean.getFolder()!=null){
        	...
        	if(str.length==1){
        		throw new Exception("不支持的路径分隔符,请用\"//\"或\"\\\"分隔你的路径");
        	}
        	//拼接文件保存目录路径
        	String path="/";
