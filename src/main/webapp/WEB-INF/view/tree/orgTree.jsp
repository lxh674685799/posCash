...
<%@include file="/commons/include/zTree.jsp" %>
	<script type="text/javascript">
		...
		function orgTreeOnClick(event, treeId, treeNode){
			if (treeNode) {
			var url="${ctx}/user/user/getUserByOrgId.do?orgId=" + treeNode.path+"&isSingle="+${param.isSingle}+"&roleId="+${param.roleId};
			$("#userListFrame").attr("src",url);
			}
		}
		...
		function selectUser(){
			...
			if(isSingle==true){
				...
				chIds = $("#userList").find(":input[name='userData']");
			}
...
<body>
	<div id="defLayout">
		...
		<div position="center">
			<div class="l-layout-header">人员列表</div>
			<iframe id="userListFrame" name="userListFrame" height="95%" width="100%"	frameborder="0" src=""></iframe>
		</div>
		<c:if test="${param.isSingle==false}">
			   <div position="right" title="<span><a onclick='javascript:dellAll();' class='link del'>清空 </a></span>" style="overflow: auto;height:95%;width:180px;">
